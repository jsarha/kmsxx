...
int main()
{
	...
	for (auto pipe : card.get_connected_pipelines())
	{
		...
		auto fb1 = new DumbFramebuffer(card, mode.get_hdisplay(), mode.get_vdisplay(), PixelFormat::XRGB8888);
		auto fb2 = new DumbFramebuffer(card, mode.get_hdisplay(), mode.get_vdisplay(), PixelFormat::XRGB8888);
